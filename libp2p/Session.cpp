    ...
    m_ping(chrono::steady_clock::time_point::max())
{
    ...
    m_netDetailLogger.add_attribute(
        "Suffix", boost::log::attributes::constant<std::string>(remoteInfoStream.str()));
    m_netErrorLogger.add_attribute(
        "Suffix", boost::log::attributes::constant<std::string>(remoteInfoStream.str()));

    m_capLogger.add_attribute(
        "Suffix", boost::log::attributes::constant<std::string>(remoteInfoStream.str()));
    m_capDetailLogger.add_attribute(
        "Suffix", boost::log::attributes::constant<std::string>(remoteInfoStream.str()));
...
bool Session::readPacket(uint16_t _capId, unsigned _packetType, RLP const& _r)
{
    ...
    LOG(m_netDetailLogger) << "Received " << capabilityPacketTypeToString(_packetType) << "("
                           << _packetType << ") from";
    ...
    {
        LOG(m_netErrorLogger) << "Exception caught in p2p::Session::interpret(): " << _e.what()
...
{
    LOG(m_capDetailLogger) << p2pPacketTypeToString(_t) << " from";
    switch (_t)
    {
...
void Session::send(bytes&& _msg)
{
    ...
    LOG(m_netDetailLogger) << "Sending " << capabilityPacketTypeToString(_msg[0]) << " to";
    if (!checkPacket(msg))
        LOG(m_netErrorLogger) << "INVALID PACKET CONSTRUCTED!";
...
{
    LOG(m_netDetailLogger) << "Disconnecting (our reason: " << reasonOf(_reason) << ") from";
...
void Session::doRead()
{
    ...
    ba::async_read(m_socket->ref(), boost::asio::buffer(m_data, h256::size),
        [this, self](boost::system::error_code ec, std::size_t length) {
            ...
            {
                LOG(m_netErrorLogger) << "header decrypt failed";
            ...
            {
                LOG(m_netErrorLogger) << "Exception decoding frame header RLP: " << _e.what() << " "
            ...
            ba::async_read(m_socket->ref(), boost::asio::buffer(m_data, tlen),
                [this, self, hLength, hProtocolId, tlen](
                    ...
                    {
                        LOG(m_netErrorLogger) << "frame decrypt failed";
                        drop(BadProtocol);  // todo: better error
                        return;
                    }
                    ...
                    {
                        LOG(m_netErrorLogger) << "Received invalid message " << frame.size() << ": "
                                              << toHex(frame) << endl;
                        disconnect(BadProtocol);
                        return;
                    }
                    else
                    {
                        ...
                        if (!ok)
                            LOG(m_netErrorLogger) << "Couldn't interpret packet. " << RLP(r);
                    }
                    doRead();
                });
        });
...
bool Session::checkRead(std::size_t _expected, boost::system::error_code _ec, std::size_t _length)
{
    ...
    {
        LOG(m_netDetailLogger) << "Error reading: " << _ec.message();
        drop(TCPError);
        return false;
    }
...
{
    LOG(m_netDetailLogger) << "DISABLE: Disabling capability '" << _capabilityName
                           << "'. Reason: " << _problem;
